...
public class ShiftPitchSynchronous {
    ...
    public ShiftPitchSynchronous(double factor, double sampleRate, int size, int overlap){

        converter = TarsosDSPAudioFloatConverter.getConverter(WavesynConstants.TARSOS_MONO_WAV);
    ...
    public byte[] process(byte[] source) {
        ...
        /* ***************** ANALYSIS ******************* */
        fftData = new float[source.length/2];
        converter.toFloatArray(source, fftData);
        ...
        byte[] shifted = new byte[outputAccumulator.length * 2];
        converter.toByteArray(outputAccumulator, shifted );
